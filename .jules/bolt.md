...
## 2024-05-24 - Reuse HTTP Client in Loop
**Learning:** Instantiating `httpx.Client` (or similar HTTP clients) inside a loop creates a new connection pool for every iteration, incurring repeated TCP/SSL handshake overhead. This is especially costly for high-latency connections.
**Action:** Lift the client creation out of the loop and pass the shared instance to the worker functions, ensuring it is thread-safe or used sequentially.
