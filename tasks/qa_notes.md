...
- **Build & Tests**: `uv sync --all-extras && uv run pytest tests/ -q` — **339
  passed** (2 subtests).
- **Code Quality**: `uv run ruff check .` — all checks passed; `uv run mypy .` —
...
- **email-security-pipeline**: Test count stable at 622; all pass with
  `requirements-ci.txt`.
- **Hydrograph**: Fresh environments need `pip install -r requirements.txt`
