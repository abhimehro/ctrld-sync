...
0. **Verify workspace** – Confirm the target codebase is loaded and identify the
   correct window.
1. **Analyze** – Use `builder` with `response_type: "review"` to study recent
   changes and find refactor opportunities.
2. **Implement** – Use `builder` with `response_type: "plan"` to implement the
...
- 🚫 **CRITICAL:** This workflow requires TWO `builder` calls – one for analysis
  (Step 1), one for implementation (Step 2). Do not skip either.
...
- 🚫 Skipping Step 2's `builder` call with `response_type: "plan"` and
  implementing without a plan
- 🚫 Doing extensive exploration (5+ tool calls) before the first `builder` call
...
- 🚫 Implementing refactorings after only the analysis phase – you need the
  second `builder` call for implementation planning
