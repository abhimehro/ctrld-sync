...
def main():
    ...
    for run in range(3):
        ...
        for i, delay in enumerate(with_jitter):
            base = base_delay * (2**i)
            print(
                f"    Attempt {i + 1}: {delay:6.2f}s (base: {base:4.1f}s, range: [{base * 0.5:.1f}s, {base * 1.5:.1f}s])"
            )
    ...
    print(
        f"  At t=1s: ALL {num_clients} clients retry simultaneously → server overload"
    )
    print(
        f"  At t=2s: ALL {num_clients} clients retry simultaneously → server overload"
    )
    print(
        f"  At t=4s: ALL {num_clients} clients retry simultaneously → server overload"
    )
    print()
    ...
    print(
        f"  First retry window: {min_time:.2f}s to {max_time:.2f}s (spread: {max_time - min_time:.2f}s)"
    )
    ...
    if window_size > 0:
        ...
        print(
            f"  Approximate peak concurrent retries with jitter: {peak_with_jitter} (per {bucket_size:.1f}s)"
        )
        ...
        print(
            "  All retries occurred at the same time; no observable spreading in this run."
        )
    print()
