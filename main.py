...
def format_duration(seconds: float) -> str:
    """Formats duration into a human-readable string (e.g., 1h 2m 3s)."""
    if seconds < 60:
    ...
    if hours > 0:
        return f"{hours}h {minutes}m {seconds_rem}s"
...
def print_summary_table(
    ...
    def make_separator(left, mid, right, horiz):
        parts = [
            horiz * (w_profile + 2),
            horiz * (w_folders + 2),
            horiz * (w_rules + 2),
    ...
    dummy_sep = make_separator(Box.TL, Box.T, Box.TR, Box.H)
    ...
    # 3. Header Separator
    print(make_separator(Box.L, Box.T, Box.R, Box.H))
    ...
    print(make_separator(Box.L, Box.X, Box.R, Box.H))
    ...
    for res in sync_results:
        ...
        s_rules = f"{res['rules']:,}"
        s_duration = format_duration(res["duration"])
    ...
    s_total_duration = format_duration(total_duration)
    ...
    # 9. Bottom Border
    print(make_separator(Box.BL, Box.B, Box.BR, Box.H))
...
def main():
    ...
    # Recalculate success for final message logic
    all_success = success_count == total
