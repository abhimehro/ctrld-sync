...
def _parse_and_cache_response(url: str, r: httpx.Response) -> dict:
    ...
    def _validate_ct() -> None:
        ...
        if not any(t in ct for t in ("application/json", "text/json", "text/plain")):
            raise ValueError(f"Invalid Content-Type from {sanitize_for_log(url)}: {sanitize_for_log(ct)}.")
        ...
        if cl:
            try:
                if int(cl) > MAX_RESPONSE_SIZE:
                    raise ValueError(f"Response too large from {sanitize_for_log(url)} ({int(cl) / (1024 * 1024):.2f} MB)")
            except ValueError as e:
                if "Response too large" in str(e):
                    raise
                log.warning(f"Malformed Content-Length header from {sanitize_for_log(url)}: {sanitize_for_log(cl)}. Falling back to streaming check.")
        chunks = []
        current_size = 0
        for chunk in r.iter_bytes(chunk_size=16 * 1024):
            current_size += len(chunk)
            if current_size > MAX_RESPONSE_SIZE:
                raise ValueError(f"Response too large from {sanitize_for_log(url)} (> {MAX_RESPONSE_SIZE / (1024 * 1024):.2f} MB)")
            chunks.append(chunk)
