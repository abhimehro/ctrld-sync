...
def format_duration(seconds: float) -> str:
    """Format duration in a human-readable way (e.g. 1m 30s)."""
    if seconds < 60:
...
def print_summary_table(
    ...
    w_profile = max(25, max_p)
    w_folders = 10
    w_rules = 12
    w_duration = 10
    w_status = 15

    total_folders = sum(r["folders"] for r in sync_results)
    total_rules = sum(r["rules"] for r in sync_results)
    total_duration = sum(r["duration"] for r in sync_results)

    all_ok = success_count == total
    if dry_run:
        total_status_text = "✅ Ready" if all_ok else "❌ Errors"
    else:
        total_status_text = "✅ All Good" if all_ok else "❌ Errors"

    total_status_color = Colors.GREEN if all_ok else Colors.FAIL
