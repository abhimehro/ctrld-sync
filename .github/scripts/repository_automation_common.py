...
def latest_tag_for_action(repo_id: str) -> str:
    # Get the latest stable release (prerelease=false in GitHub API)
    # The API actually returns a 404 for /releases/latest if there are no stable releases
    # But occasionally it might return a pre-release if it was explicitly marked "latest" via API misuse.
    # So we filter explicitly.
    latest = gh_text(
        [
            "api",
            f"repos/{repo_id}/releases/latest",
            "--jq",
    ...
    # Fallback 1: List all releases, get the first non-prerelease
    releases = gh_json(
        [
            "api",
...
def ref_exists(repo_id: str, ref: str) -> bool:
    """Check if a specific ref (tag or branch) exists in the repository."""
    # First check tags
    tag_result = run_process([GH_BIN, "api", f"repos/{repo_id}/git/refs/tags/{ref}"])
