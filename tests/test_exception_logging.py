...
class TestExceptionLogging(unittest.TestCase):
    ...
    def test_check_api_access_redacts_exception(self, mock_log):
        ...
        error_message = (
            "Connection error to https://api.controld.com?token=TEST_TOKEN_XYZ"
        )
    ...
    def test_validate_folder_url_redacts_exception(self, mock_getaddrinfo, mock_log):
        ...
        mock_getaddrinfo.side_effect = OSError(f"Failed to resolve {test_url}")
        ...
        self.assertTrue(mock_log.warning.called, "log.warning should have been called")
    ...
    def test_create_folder_redacts_exception(
        ...
        error_with_token = ValueError(f"Failed to parse response from {sensitive_url}")
        ...
        self.assertTrue(
            mock_log.debug.called, "log.debug should have been called for exception"
        )
